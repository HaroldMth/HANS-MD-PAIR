...
async function startPairingCode(phone, onSuccess, onError) {
  ...
  async function connect(isFirstConnect) {
    ...
    if (isFirstConnect) {
      // Wipe stale session data ONLY on the first connect
      try { fs.rmSync(sessionDir, { recursive: true, force: true }) } catch (_) { }
    ...
    console.log(`[PAIR-CODE] [${new Date().toLocaleTimeString()}] Initializing socket for ${phone}...`)
    if (sock.ws && typeof sock.ws.on === 'function') {
      sock.ws.on('error', (e) => console.log(`[PAIR-CODE] WS Error: ${e.message}`))
    }

    // Attach ALL listeners BEFORE any async work (mirrors gifted pattern)
    sock.ev.on('creds.update', async (update) => {
      try { saveCreds(update) } catch (e) { console.log(`[PAIR-CODE] Silenced saveCreds error: ${e.message}`) }
      // Aggressive success detection: if 'me' is present, we are logged in!
      if (update.me && !shared.settled) {
        shared.settled = true
        console.log(`[PAIR-CODE] ✅ Self-ID found early — waiting for file flush...`)
        
        // Wait 2s for filesystem to finish writing creds.json
        await new Promise(r => setTimeout(r, 2000))
        
        try {
          const credsPath = path.join(sessionDir, 'creds.json')
          // Basic retry if file still isn't there
          if (!fs.existsSync(credsPath)) {
             console.log('[PAIR-CODE] ⏳ creds.json not found yet, retrying...')
             await new Promise(r => setTimeout(r, 2000))
          }

          const sessionId = await saveToMongo(credsPath, phone)
          const jid = update.me.id || `${phone}@s.whatsapp.net`
          await sock.sendMessage(jid, { text: buildSuccessMessage(sessionId) })
          onSuccess(sessionId)
          console.log(`[PAIR-CODE] ✅ Session delivered to ${phone}`)
        } catch (err) {
          console.error('[PAIR-CODE] Early upload failed:', err.message)
          onError(err)
        } finally {
          setTimeout(() => {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        ...
        try {
          ...
          // Wait 3s for message delivery before closing
          setTimeout(() => {
            try { if (sock.ws?.readyState === 1) sock.end(undefined) } catch (_) { }
            fullCleanup(false) 
          }, 3000)
      ...
      if (connection === 'close') {
        ...
        console.log(`[PAIR-CODE] Reconnecting in ${delay / 1000}s (attempt ${shared.reconnectCount}/${MAX_RECONNECTS})... Reason: ${is515 ? '515/Restart' : 'Standard Close'}`)
