...
async function startPairingCode(phone, onSuccess, onError) {
  ...
  async function connect(isFirstConnect = true) {
    ...
    sock.ev.on('creds.update', async (update) => {
      ...
      if (sock.authState.creds.registered && sock.authState.creds.me && !shared.sessionId && !shared.done) {
        console.log(`[PAIR-CODE] ✅ Authenticated as ${sock.authState.creds.me.id} — saving to MongoDB...`)
        try {
...
async function startQRSession(sessionKey, onQR, onSuccess, onError) {
  ...
  async function connect(isFirstConnect = true) {
    ...
    sock.ev.on('creds.update', async (update) => {
      ...
      if (sock.authState.creds.registered && sock.authState.creds.me && !shared.sessionId && !shared.done) {
        console.log(`[QR] ✅ Authenticated as ${sock.authState.creds.me.id} — saving to MongoDB...`)
        try {
          const credsPath = path.join(sessionDir, 'creds.json')
          let attempts = 0
